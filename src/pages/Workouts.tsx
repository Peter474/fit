...
export default function Workouts() {
  ...
  function handleSaveName() {
    saveWorkout({ ...log.workout, name: nameDraft.trim() })
    setNameEditing(false)
  }

  function handleAddExercise(exercise: Omit<Exercise, 'id'>) {
    const entry: Exercise = { ...exercise, id: newId() }
    saveWorkout({ ...log.workout, exercises: [...log.workout.exercises, entry] })
  }
  ...
  function handleDeleteExercise(id: string) {
    saveWorkout({ ...log.workout, exercises: log.workout.exercises.filter((ex) => ex.id !== id) })
  }

  function handleMarkStatus(status: WorkoutStatus) {
    saveWorkout({ ...log.workout, status: log.workout.status === status ? 'none' : status })
  }
